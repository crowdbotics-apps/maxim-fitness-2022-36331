    ...
    <View style={styles.mainContainer}>
      <View style={styles.card}>
        <View style={styles.cardHeader}>
          <Image source={item && item.user && item.user.profile_picture_url ? item.user.profile_picture_url : Images.profile} style={styles.profileImg} />
          <View style={styles.username}>
            <Text text={item && item.user && item.user.username ? item.user.username : "Orum_training_oficial"} style={styles.text1} />
        ...
        <View style={styles.cardBody}>
          <SliderBox
            images={item && item?.post_image_video?.length > 0 ? item.post_image_video.map((item) => item.image) : []}
        ...
        <View style={styles.cardHeader1}>
          <View style={styles.socialIcons}>
            ...
            <Text text={item && item.comments && item.comments.length ? item.comments.length : null} style={styles.text2} />
          </View>
          <Pressable style={styles.socialIcons} onPress={addLikeAction}>
            <Image source={Images.heartIcon} style={styles.socialImg} tintColor={item.liked ? 'red' : 'black'} />
...
const styles = StyleSheet.create({
  ...
  cardBody: {flex: 1, justifyContent: 'center', alignItems: 'center', marginTop: 10, marginBottom: 30, borderRadius: 15},
  text1: {fontSize: 15, marginLeft: 10},
  text2: {fontSize: 15, marginLeft: 10},
  username: {flexDirection: 'row', alignItems: 'center', flex: 1},
  socialIcons: {flexDirection: 'row', justifyContent: 'center', alignItems: 'center', flex: 1},
  ...
  seeMoreStyle: {paddingVertical: 5, fontSize: 15, lineHeight: 16, color: 'blue'},
