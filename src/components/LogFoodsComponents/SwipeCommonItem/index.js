...
const SwipeCommonItem = ({
  ...
  const calculateCalories = obj => {
    const data = (obj.serving_weight / item.serving_weight_grams) * item.full_nutrients.length;
    setCalories(data);
  };
      ...
      >
        <View style={styles.mainUpperContainer}>
          ...
          <View style={styles.mainInputQtyWrapper}>
            ...
            <View style={styles.qtyParentWrapper}>
              <SubDropDown item={item?.alt_measures} onSelect={onSelect} unitText={unitText} />
            </View>
          </View>
        </View>
        <View style={[row, smallBMargin]}>
          ...
          <View style={styles.caloriesContainer}>
            <Text style={styles.caloriesText}>{Math.ceil(caloriesCalc) || 0}</Text>
...
export default SwipeCommonItem;
