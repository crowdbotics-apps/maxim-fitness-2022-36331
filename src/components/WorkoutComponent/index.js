...
const WorkoutComponent = ({
  ...
  return (
    <>
      ...
      >
        ...
        >
          <TouchableOpacity onPress={() => setIsVisible(!isVisible)}>
            <View style={[regularVMargin, { borderWidth: 1, width: 40, height: 1 }]} />
          </TouchableOpacity>
          <View style={row}>{data && <Text text={data.exercise.name} smallTitle bold />}</View>
          <View
            style={{
              flex: 1,
              overflow: 'hidden',
      ...
      >
        <TouchableOpacity style={fill} onPress={() => setDetailModal(!detailModal)} />
        <View
          style={[
            fill3x,
            secondaryBg,
        ...
        >
          <View style={alignItemsCenter}>
            ...
            <View style={row}>{data && <Text text={data.exercise.name} smallTitle bold />}</View>
            <View style={{ backgroundColor: '#D3D3D3', borderRadius: 20, marginTop: 10 }}>
              <Image
                source={{ uri: data?.exercise?.pictures[0]?.image_url }}
                style={{
                  width: 200,
